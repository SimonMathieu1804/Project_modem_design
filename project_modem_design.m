...
mu = 0.5; %0.5
Pi = zeros(1,N);
N0 = 1.28*f_sub;%6.66E-7;%0.000035;
...
for (n=0:1000000)
    Pi = (mu*(abs(Hf).^2)-N0*f_sub)./(abs(Hf).^2);
    Pi0 = Pi>0;
    Pi = Pi0.*Pi;
    Ptot = sum(Pi);
...
BitsWF = 1/2*log(1+(Pi.*(abs(Hff).^2)./(N0*f_sub)*Gamma))/log(2);%
%-Power uniformly distributed
BitsPowerUniform = 1/2*log(1+(Pmax/Nb*ones(1,Nb).*(abs(Hff).^2)./(N0*f_sub)*Gamma))/log(2);%.*(abs(Hff).^2 
...
Gamma = 2/3*((erfcinv(Petarg/2))^2);
%- Water-filling distribution power
BitsBonus = 1/2*log(1+(Pi.*(abs(Hff).^2))./(N0.*(abs(Hff).^2)*Gamma))/log(2);%
%-Power uniformly distributed
